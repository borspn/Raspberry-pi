...
func InitSPI(chipName string, csGPIO int, spiDev string) {
	...
	_, _, errno := syscall.Syscall(syscall.SYS_IOCTL, f.Fd(),
		spiIOCWrMode, uintptr(unsafe.Pointer(&mode1)))
	if errno != 0 {
		log.Fatalf("Failed to set SPI mode: %v", errno)
	}
	// bits
	_, _, errno = syscall.Syscall(syscall.SYS_IOCTL, f.Fd(),
		spiIOCWrBitsPerWord, uintptr(unsafe.Pointer(&bits8)))
	if errno != 0 {
		log.Fatalf("Failed to set SPI bits-per-word: %v", errno)
	}
	// speed
	_, _, errno = syscall.Syscall(syscall.SYS_IOCTL, f.Fd(),
		spiIOCWrMaxSpeedHz, uintptr(unsafe.Pointer(&speed500kHz)))
	if errno != 0 {
	...
	_, _, errno = syscall.Syscall(syscall.SYS_IOCTL, chipDev.Fd(),
		uintptr(GPIO_GET_LINEHANDLE_IOCTL), uintptr(unsafe.Pointer(&req)))
	if errno != 0 {
...
func putCSLow() {
	...
	if errno != 0 {
		log.Fatalf("Failed to put CS line low: %v", errno)
	}
}
...
func putCSHigh() {
	...
	if errno != 0 {
		log.Fatalf("Failed to put CS line high: %v", errno)
	}
}
